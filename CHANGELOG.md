# Changelog

All notable changes to KissConfig are documented here.

## Unreleased

- Unreleased changes will be documented here.

## 0.1.0 - 2026-05-08

- Initial project structure for the `io.github.arthurhoch:kiss-config` Maven artifact.
- Added zero-dependency configuration loading for `.properties`, explicit `.env` files, Java system properties, and operating-system environment variables.
- Added Java record mapping, interpolation, merge strategies, source reporting, documentation, GitHub workflows, and Maven Central release preparation.
- Expanded release-ready documentation, AI project context, examples, and Javadocs before the first release.
