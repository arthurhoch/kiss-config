...
KissConfig uses free GitHub-native and open-source checks. Normal CI, CodeQL, Dependency Review, Dependabot, and OpenSSF Scorecard do not require repository secrets.
...
Before deleting code, follow [Safe Code Cleanup](code-cleanup.md). Treat public API, source ordering, merge behavior, mapping, and secret masking as compatibility-sensitive.

Before release, run the normal build, Javadocs, coverage generation, compile-scope dependency check, and any relevant optional profile. Release secrets must stay limited to the release workflow and must not be used by security workflows.
